...
pub const Repository = struct {
    ...
    pub fn init(allocator: Allocator, format: Oid.Format, pack_file: *std.fs.File.Reader, index_file: *std.fs.File.Reader) !Repository {
...
const Odb = struct {
    ...
    fn init(allocator: Allocator, format: Oid.Format, pack_file: *std.fs.File.Reader, index_file: *std.fs.File.Reader) !Odb {
        try pack_file.seekTo(0);
        try index_file.seekTo(0);
        const index_header = try IndexHeader.read(&index_file.interface);
        return .{
            .format = format,
            .pack_file = pack_file,
...
const PackHeader = struct {
    ...
    fn read(reader: *std.io.Reader) !PackHeader {
        const actual_signature = try reader.takeArray(4);
...
const EntryHeader = union(Type) {
    ...
    fn read(format: Oid.Format, reader: *std.io.Reader) !EntryHeader {
        ...
        return switch (@"type") {
            ...
            .ref_delta => .{ .ref_delta = .{
                .base_object = try Oid.readBytes(format, reader),
                .uncompressed_length = uncompressed_length,
            } },
        };
    }
...
const IndexHeader = struct {
    ...
    fn read(reader: *std.io.Reader) !IndexHeader {
        const actual_signature = try reader.takeArray(4);
        if (!mem.eql(u8, actual_signature, signature)) return error.InvalidHeader;
        const version = try reader.takeInt(u32, .big);
        if (version != supported_version) return error.UnsupportedVersion;
        ...
        for (&fan_out_table) |*entry| {
            entry.* = try reader.takeInt(u32, .big);
        }
...
/// [pack-format](https://git-scm.com/docs/pack-format).
pub fn indexPack(allocator: Allocator, format: Oid.Format, pack: *std.fs.File.Reader, index_writer: *std.fs.File.Writer) !void {
    try pack.seekTo(0);
...
fn indexPackFirstPass(
    ...
    var pack_hash_buffer: [1024]u8 = undefined;
    var pack_hashed_reader = std.io.Reader.hashed(&pack.interface, Oid.Hasher.init(format), &pack_hash_buffer);

    const pack_header = try PackHeader.read(&pack_hashed_reader.interface);

    var current_entry: u32 = 0;
    while (current_entry < pack_header.total_objects) : (current_entry += 1) {
        const entry_offset = pack.logicalPos();
        var entry_crc32_buffer: [1024]u8 = undefined;
        var entry_crc32_reader = std.io.Reader.hashed(&pack_hashed_reader.interface, std.hash.Crc32.init(), &entry_crc32_buffer);
        const entry_header = try EntryHeader.read(format, &entry_crc32_reader.interface);
        var flate_buffer: [std.compress.flate.max_window_len]u8 = undefined;
        var entry_decompress_stream: std.compress.flate.Decompress = .init(&entry_crc32_reader.interface, .zlib, &flate_buffer);
        switch (entry_header) {
            .commit, .tree, .blob, .tag => |object| {
                var entry_hash_buffer: [1024]u8 = undefined;
                var entry_hashed_reader = std.io.Reader.hashed(&entry_decompress_stream.reader, Oid.Hasher.init(format), &entry_hash_buffer);
                // The object header is not included in the pack data but is
                // part of the object's ID
                var object_header_buf: [32]u8 = undefined;
                const object_header = std.fmt.bufPrint(&object_header_buf, "{t} {d}\x00", .{ entry_header, object.uncompressed_length }) catch unreachable;
                entry_hashed_reader.hasher.update(object_header);
                const read = try entry_hashed_reader.interface.discardRemaining();
                if (read != object.uncompressed_length) {
                    return error.InvalidObject;
                }
                const oid = entry_hashed_reader.hasher.finalResult();
            ...
            inline .ofs_delta, .ref_delta => |delta| {
                const read = try entry_decompress_stream.reader.discardRemaining();
                if (read != delta.uncompressed_length) {
                    return error.InvalidObject;
                }
...
fn indexPackHashDelta(
    ...
    var entry_header_buf: [32]u8 = undefined;
    const entry_header = std.fmt.bufPrint(&entry_header_buf, "{t} {d}\x00", .{ base_object.type, base_data.len }) catch unreachable;
    entry_hasher.update(entry_header);
...
fn resolveDeltaChain(
    ...
    while (i > 0) {
        ...
        var expanded_delta_writer: std.io.Writer = .fixed(expanded_data);
        var base_reader: std.io.Reader = .fixed(base_data);
        try expandDelta(&base_reader, &delta_reader, &expanded_delta_writer);
        if (expanded_delta_writer.end != expanded_size) return error.InvalidObject;
...
/// [pack-format](https://git-scm.com/docs/pack-format).
fn expandDelta(base_object: *std.io.Reader, delta_reader: *std.io.Reader, writer: *std.io.Writer) !void {
    while (true) {
        const inst: packed struct { value: u7, copy: bool } = @bitCast(try delta_reader.takeByte());
        if (inst.copy) {
            ...
            if (offset > base_object.end) return error.InvalidDeltaInstruction;
            base_object.seek = offset;
            if (size > base_object.bufferedLen()) return error.InvalidDeltaInstruction;
            try writer.writeAll(base_object.buffered()[0..size]);
        } else if (inst.value != 0) {
            if (inst.value > delta_reader.bufferedLen()) return error.InvalidDeltaInstruction;
            try writer.writeAll(delta_reader.buffered()[0..inst.value]);
        } else {
...
fn runRepositoryTest(comptime format: Oid.Format, head_commit: []const u8) !void {
    ...
    var repository = try Repository.init(testing.allocator, format, &pack_file_reader, &index_file_reader);
