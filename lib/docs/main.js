(function() {
    ...
    const curNav = {
      ...
      viewSourceHash: null,
      // the decl to scroll to when viewing the source
      viewSourceScrollDecl: -1,
    };
    var curNavSearch = "";
    ...
    function render() {
        ...
        switch (curNav.tag) {
          ...
          case 2: return renderSource(curNav.path, curNav.viewSourceScrollDecl);
    ...
    function renderSource(path, scroll_decl_index = -1) {
      ...
      domSourceText.innerHTML = declSourceHtml(decl_index, scroll_decl_index);

      domSectSource.classList.remove("hidden");

      // The innerHTML update above does not seem to reflect in the DOM
      // immediately; we need to wait until the next event cycle before the
      // scroll anchor will be present.
      setTimeout(() => document.getElementById("scroll-anchor")?.scrollIntoView?.());
    }

    function renderDeclHeading(decl_index) {
      curNav.viewSourceHash = "#src/" + unwrapString(wasm_exports.decl_file_path(decl_index));
      curNav.viewSourceScrollDecl = decl_index;
    ...
    function declSourceHtml(decl_index, scroll_decl_index = -1) {
      return unwrapString(wasm_exports.decl_source_html(decl_index, scroll_decl_index));
    }
