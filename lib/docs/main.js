(function() {
    ...
    const domListModules = document.getElementById("listModules");
    ...
    const domSectModules = document.getElementById("sectModules");
    ...
    function renderTitle() {
      ...
      if (curNavSearch.length > 0) {
        ...
        // Home
        const moduleCount = wasm_exports.module_count();
        if (moduleCount === 0) {
          document.title = "Error" + suffix;
        } else if (moduleCount === 1) {
          const moduleName = unwrapString(wasm_exports.module_name(0));
          document.title = moduleName + suffix;
        } else {
          document.title = "Modules" + suffix;
        }
      }
    }
    ...
    function render() {
        ...
        domSectModules.classList.add("hidden");
    ...
    function renderHome() {
      ...
      if (moduleCount === 0) {
        ...
        resizeDomList(domListModules, moduleCount, '<li><a href="#"></a></li>');
        for (let i = 0; i < moduleCount; i += 1) {
          const liDom = domListModules.children[i];
          const aDom = liDom.children[0];
          const moduleName = unwrapString(wasm_exports.module_name(i));
          aDom.textContent = moduleName;
          aDom.setAttribute('href', '#' + moduleName);
        }
        domSectModules.classList.remove('hidden');
      }
    }
