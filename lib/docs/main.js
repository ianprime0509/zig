(function() {
    ...
    function renderTitle() {
      ...
      if (curNavSearch.length > 0) {
        ...
        document.title = fullyQualifiedName(curNav.decl, false) + suffix;
    ...
    function renderNavFancy(cur_nav_decl, list) {
      {
        ...
        if (prev_decl_it != null) {
          const file_path = fullyQualifiedName(prev_decl_it, true);
    ...
    function navLinkDeclIndex(decl_index) {
      return navLinkFqn(fullyQualifiedName(decl_index, true));
    }
    ...
    function renderSearch() {
        ...
        if (results.length !== 0) {
            ...
            for (let i = 0; i < results.length; i += 1) {
                ...
                aDom.textContent = fullyQualifiedName(match, false);
                aDom.setAttribute('href', navLinkFqn(fullyQualifiedName(match, true)));
            }
            renderSearchCursor();
    ...
    function fullyQualifiedName(decl_index, encode) {
      return unwrapString(wasm_exports.decl_fqn(decl_index, encode));
    }
